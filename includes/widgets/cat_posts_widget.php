...
function __construct() {
		parent::__construct(
			'cat-post', // Base ID
			__( 'Recent Post and Thumbnails by Category', 'doubledown' ), // Name
			array( 'description' => __( 'Display recent posts with thumbnail by category.', 'doubledown' ), ) // Args
		);
	}		
...
<p>
<label for="<?php echo $this->get_field_id('numsites'); ?>"><?php _e('Number of Posts to Show', 'doubledown'); ?>:</label>
...
<p>
	<label><?php _e('Filter By Tag (Optional)', 'doubledown'); ?></label>
    <?php
...
<p>
	<label><?php _e('Image Align Left or Right', 'doubledown'); ?></label>

    <select style="width:60%;" class="widefat" name="<?php echo $this->get_field_name( 'gallalign' ); ?>">
    
    	<option <?php if ($instance['gallalign']== 'Left') { echo "SELECTED"; }?> value="Left">Left</option>
	<option <?php if ($instance['gallalign']== 'Right') { echo "SELECTED"; }?> value="Right">Right</option>
