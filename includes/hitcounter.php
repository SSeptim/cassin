...
{
	add_submenu_page('design-options',__('Hit Statistics', 'doubledown'), __('Hit Statistics', 'doubledown'), 'update_themes', 'hits-options', 'hits_show_ui');
}
...
?>
	<div class="wrap meta-box-sortables">
        ...
        <h2><?php _e('Outbound Casino Link Hits', 'doubledown'); ?></h2>
        ...
        <table cellspacing="0" class="wp-list-table widefat fixed posts" style="width:500px !important">
            <thead>
            	<tr>
                	<th style="" class="manage-column column-title" id="title" scope="col"><?php _e('Title', 'doubledown'); ?></th>
                	<th style="width:50px" class="manage-column column-description" id="description" scope="col"><?php _e('Hits', 'doubledown'); ?></th>
                    <th style="width:50px"><?php _e('Reset?', 'doubledown'); ?></th>
                </tr>
            </thead>
            ...
            <tfoot>
				<tr>
                	<th style="" class="manage-column column-title" id="title" scope="col"><?php _e('Title', 'doubledown'); ?></th>
                	<th style="" class="manage-column column-description" id="description" scope="col"><?php _e('Hits', 'doubledown'); ?></th>
                    <th><?php _e('Reset?', 'doubledown'); ?></th>
                </tr>
            </tfoot>
            	...
            	<tr valign="top" class="post-<?php echo $p->ID;?> casino type-casino status-publish hentry  iedit author-other" id="post-<?php echo $p->ID;?>">
					<td class="post-title page-title column-title">
						...
						<div class="row-actions">
                        	<span class="edit"><a title="Edit this item" href="<?php echo get_bloginfo( 'url' );?>/wp-admin/post.php?post=<?php echo $p->ID;?>&amp;action=edit"><?php _e('Edit', 'doubledown'); ?></a> | </span>
                            <span class="view"><a rel="permalink" title="View <?php echo $p->post_title;?>" href="<?php echo get_permalink($p->ID);?>"><?php _e('View', 'doubledown'); ?></a></span>
                        </div>
                    </td>
					<td class="flytonic_hits column-flytonic_hits"><?php echo intval(get_post_meta($p->ID, '_flytonic_hitcounter', true));?></td>
                    <td><a href="?page=hits-options&action=clear&id=<?php echo $p->ID;?>">[<?php _e('Clear', 'doubledown'); ?>]</a></td>
				</tr>
			<?php endforeach; ?>    
		</tbody>
</table>
...
<form class="form-wrap" method="post" action="?page=hits-options&action=clearall">
                    ...
                    <input type="submit" value="<?php _e('Clear ALL Hit Counts', 'doubledown'); ?>" accesskey="p" tabindex="5" id="publish" class="button-primary" name="clearall">
                    </form>
