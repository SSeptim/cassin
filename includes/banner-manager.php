...
{
	add_submenu_page('design-options', __('Banner Manager', 'doubledown'), __('Banner Manager', 'doubledown'), 'update_themes', 'banner-manager', 'banner_manager_show_ui');
}
...
{	
	...
	<div class="wrap meta-box-sortables">
        ...
        <h2><?php _e('Create Banner in', 'doubledown'); ?><?php echo $group->title; ?></h2>
        <form method="post" action="?page=banner-manager&section=new-banner&group=<?php echo $group->slug ?>" style="width: 400px;">
            <div class="form-field">
	            <label><?php _e('Title', 'doubledown'); ?></label>
    	        <input type="text" name="banner-title" />
        	    <p><?php _e('A name for the banner so you can quickly identify banners later', 'doubledown'); ?></p>
            </div>
            <div class="form-field">
            	<input class="check" name="banner-type" type="radio" value="simple" CHECKED /><?php _e('Simple Banner', 'doubledown'); ?> <br />
			</div>
                <div id="simple-banner" style="padding-left: 50px; padding-bottom: 5px; margin-bottom: 5px; border-bottom: 1px solid #CCC">
       			    <div class="form-field">
            			<label><?php _e('Image', 'doubledown'); ?></label>
					    ...
					    <input class="clear_field_button button-secondary" type="button" value="<?php _e('Clear Field', 'doubledown'); ?>" />
                        <p><?php _e('Upload an image to use as your banner', 'doubledown'); ?></p>
					</div>
                    <div class="form-field">
                    	<label><?php _e('Link', 'doubledown'); ?></label>
                        <input type="text" name="banner-link" />
                        <p><?php _e('Enter the URL the banner should link to', 'doubledown'); ?></p>
                    </div> 
					<div class="form-field">
                    	<label><?php _e('Alt Text', 'doubledown'); ?></label>
                        <input type="text" name="banner-alt" />
                        <p><?php _e('Alt text to be used on the link and image', 'doubledown'); ?></p>
                    </div> 
                </div>
           <div class="form-field">
           		<input class="check" name="banner-type" type="radio" value="html" /> <?php _e('HTML Banner', 'doubledown'); ?>
           </div>
				<div id="html-banner" style="padding-left: 50px;">
       			    <div class="form-field">
            			<label><?php _e('Banner HTML', 'doubledown'); ?></label>
                        <textarea rows=10 name="banner-html"></textarea>
                        <p><?php _e('Paste HTML code from a banner exchange or affiliate program', 'doubledown'); ?></p>
					</div>
                </div>
                
			<input type="submit" name="action" class="button-primary" id="publish" value="<?php _e('Create Banner', 'doubledown'); ?>">
			<input type="submit" name="action" class="button-secondary" id="publish" value="<?php _e('Cancel', 'doubledown'); ?>">
...
function banner_manager_show_edit_banner($group, $bannerIndex, $banner_groups)
{
	...
	<div class="wrap meta-box-sortables">
    	<div class="icon32" id="icon-themes"><br></div>
        <h2><?php _e('Edit Banner', 'doubledown'); ?></h2>
        <form method="post" action="?page=banner-manager&section=edit-banner&group=<?php echo $group->slug ?>&banner-index=<?php echo $bannerIndex;?>" style="width: 400px;">
            ...
            <div class="form-field">
            	<input class="check" name="banner-type" type="radio" value="simple" <?php echo ($banner->html==""?"CHECKED":"");?> /> <?php _e('Simple Banner', 'doubledown'); ?><br />
			</div>
                <div id="simple-banner" style="padding-left: 50px; padding-bottom: 5px; margin-bottom: 5px; border-bottom: 1px solid #CCC">
       			    <div class="form-field">
            			<label><?php _e('Image', 'doubledown'); ?></label>
           ...
           <div class="form-field">
           		<input class="check" name="banner-type" type="radio" value="html" <?php echo ($banner->html!=""?"CHECKED":"");?> /> <?php _e('HTML Banner', 'doubledown'); ?>
           </div>
				<div id="html-banner" style="padding-left: 50px;">
       			    <div class="form-field">
            			<label><?php _e('Banner HTML', 'doubledown'); ?></label>
			...
			<input type="submit" name="action" class="button-primary" id="publish" value="<?php _e('Save Banner', 'doubledown'); ?>">
			<input type="submit" name="action" class="button-secondary" id="publish" value="<?php _e('Cancel', 'doubledown'); ?>">
...
jQuery(document).ready( function() {
	...
	<div class="wrap meta-box-sortables">
    	<div class="icon32" id="icon-themes"><br></div>
        <h2><?php _e('Banner Manager', 'doubledown'); ?></h2>
        
        <div id="poststuff">
        
        <h3><?php _e('Create a group', 'doubledown'); ?></h3>
        <p><?php _e('An ad group contains one or more advertisements to be displayed on your site in a particular location. To create a new group begin by typing a name below and pressing "create"', 'doubledown'); ?>
        <form method="post" action="?page=banner-manager">
        	<label><?php _e('Group Title:', 'doubledown'); ?></label>
            <input type="text" name="group_title" /> 
            <input type="submit" value="Create Group" id="publish" class="button-primary" name="action" />
        </form>
        <hr />
        <p><?php _e('Each group can contain one or more advertisements. Advertisements will be selected randomly from the group, and you may selectively disable individual advertisements to stop them from appearing temporarily.', 'doubledown'); ?></p>
        <?php foreach ($banner_groups as $group) : ?>
            ...
            <?php _e('Template Tag:', 'doubledown'); ?> &lt;?php if (function_exists('bannerAd')) bannerAd('<?php echo $group->slug;?>'); ?&gt;</p>
            
            <table cellspacing="0" class="widefat fixed" style="width: auto">
                <thead>
                    <tr>
                    <th class="manage-column"><?php _e('Active', 'doubledown'); ?></th>
                    <th class="manage-column" style="width: 150px"><?php _e('Title', 'doubledown'); ?></th>
                    <th class="manage-column" style="width: 75px"><?php _e('Type', 'doubledown'); ?></th>
                    <th class="manage-column" style="width: 50px"><?php _e('Hits', 'doubledown'); ?></th>
                    <th class="manage-column" style="width: 150px"><?php _e('Key', 'doubledown'); ?></th>
                    <th class="manage-column" style="width: 200px"><?php _e('Actions', 'doubledown'); ?></th>
                    </tr>
                </thead>
                ...
                <tfoot>
                    <tr>
                    <th class="manage-column"><?php _e('Active', 'doubledown'); ?></th>
                    <th class="manage-column"><?php _e('Title', 'doubledown'); ?></th>
                    <th class="manage-column"><?php _e('Type', 'doubledown'); ?></th>
                    <th class="manage-column"><?php _e('Hits', 'doubledown'); ?></th>
                    <th class="manage-column"><?php _e('Key', 'doubledown'); ?></th>
                    <th class="manage-column"><?php _e('Actions', 'doubledown'); ?></th>
                    </tr>
                </tfoot>
                ...
                <?php 
                	...
                	<tr>
                        ...
                        <td>
                        	<a href="?page=banner-manager&section=edit-banner&group=<?php echo $group->slug;?>&banner-index=<?php echo $banner_key;?>"><?php _e('Edit', 'doubledown'); ?></a> 
                            <a href="?page=banner-manager&section=delete-banner&group=<?php echo $group->slug;?>&banner-index=<?php echo $banner_key;?>"><?php _e('Delete', 'doubledown'); ?></a> 
                            <?php if ($banner->active) : ?>
                            <a href="?page=banner-manager&group=<?php echo $group->slug;?>&banner-index=<?php echo $banner_key;?>&action=deactivate"><?php _e('Deactivate', 'doubledown'); ?></a>
                            <?php else: ?>
                            <a href="?page=banner-manager&group=<?php echo $group->slug;?>&banner-index=<?php echo $banner_key;?>&action=activate"><?php _e('Activate', 'doubledown'); ?></a>                        
                            <?php endif; ?>
                            <a href="?page=banner-manager&group=<?php echo $group->slug;?>&banner-index=<?php echo $banner_key;?>&action=reset-hits"><?php _e('Reset Hits', 'doubledown'); ?></a>                        
                        
                        </td>
                    </tr>
                <?php $y++; endforeach; ?>
					<tr>
                        <td colspan=4><a href="?page=banner-manager&section=new-banner&group=<?php echo $group->slug; ?>"><?php _e('Create new banner', 'doubledown'); ?></a></td>
...
<p>
<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title (Optional):', 'doubledown'); ?></label>
<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" type="text" style="width:100%;" />
            ...
            <p>
                ...
                <select class="widefat" id="<?php echo $this->get_field_id('banner_title'); ?>" name="<?php echo $this->get_field_name('banner_title'); ?>">
                	<option value=""><?php _e('Random', 'doubledown'); ?></option>
                    <?php foreach ($banner_groups as $group) :?>
